// Buscador de catálogo compartido (cotizador / remisiones) — Ola 1-A del
// editor híbrido de remisiones (spec 2026-08-04-remision-editor-hibrido-design.md).
//
// Extraído de `features/cotizador/components/ProductSearch.tsx`. Es 100%
// prop-based: NO lee ningún store — todo lo que antes disparaba al store del
// cotizador ahora entra por callbacks. El cotizador lo consume vía el wrapper
// delgado `ProductSearch` (mismo archivo/import de siempre) que conecta su
// store; la remisión pasa sus propios callbacks.
//
// Los hooks de búsqueda (useProductosSearch / useFantasmasSearch) y
// CatalogoFiltros son solo queries/presentación sin estado de documento, por
// eso se importan directo desde el feature cotizador sin duplicarlos.
import { useState, useEffect, useRef, useCallback } from 'react';
import { useDismiss } from '@/lib/useDismiss';
import { Search, Package, Sparkles, Ghost, Wrench } from 'lucide-react';
import { Input } from '@/components/ui/input';
import type { ApiError } from '@/lib/api';
import { useProductosSearch } from '@/features/cotizador/hooks/useProductosSearch';
import { useFantasmasSearch, type FantasmaPrevio } from '@/features/cotizador/hooks/useFantasmasSearch';
import { CatalogoFiltros } from '@/features/cotizador/components/CatalogoFiltros';
import type { Producto, Servicio } from '@/features/cotizador/types';

// Re-export de conveniencia para consumidores del panel.
export type { Producto, Servicio, FantasmaPrevio };

export type ProductSearchPanelProps = {
  /** Producto del catálogo elegido (qty = cantidad parseada del query o 1). */
  onAddProducto: (p: Producto, qty: number) => void | Promise<void>;
  /** Servicio del catálogo elegido. */
  onAddServicio: (s: Servicio, qty: number) => void;
  /** Fantasma previo (línea ad-hoc ya solicitada antes) elegido. */
  onAddFantasma: (f: FantasmaPrevio, qty: number) => void;
  /** Abre el flujo "capturar fantasma nuevo" (botón + empty state). */
...
export function ProductSearchPanel({
  ...
  async function onSelect(p: Producto) {
    await onAddProducto(p, cantidadParseada ?? 1);
    setQ('');
    setOpen(false);
  ...
  function onSelectServicio(s: Servicio) {
    onAddServicio(s, cantidadParseada ?? 1);
    setQ('');
    setOpen(false);
    setTimeout(() => inputRef.current?.focus(), 0);
  }
  ...
  function onSelectFantasma(f: FantasmaPrevio) {
    onAddFantasma(f, cantidadParseada ?? 1);
    setQ('');
    setOpen(false);
    setTimeout(() => inputRef.current?.focus(), 0);
  }
  ...
  return (
    <div className="relative" ref={containerRef}>
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          type="button"
          onClick={() => onOpenAddFantasma({ initialDescripcion: '' })}
                    ...
                    <div className="p-3 text-center space-y-2">
                      ...
                      {q.trim() && (
                        <button
                          type="button"
                          onClick={() => {
                            onOpenAddFantasma({ initialSku: q.trim() });
                            setOpen(false);
                            setQ('');
                          }}
