...
- **Responsive:** cero fallbacks móviles tipo card en tablas de features (0 resultados de `md:hidden`/`hidden md:block` en features). *Corrección 2026-08-04:* el carrito del cotizador SÍ tiene card móvil completa (`components/document/DocumentCartTable.tsx:71` + `DocumentRow.tsx:349`, fuera de features/ — por eso el grep no la vio); el bloqueador móvil real del cotizador son header y pickers de ancho fijo — ver `docs/product/cotizador-mobile-plan.md`. *Nota re-evaluada 2026-08-04:* el `min-w-[640px]` móvil de `ui/data-table.tsx:22` NO está invertido — su wrapper ya es `overflow-x-auto`, así que es el fallback estándar de scroll interno que evita columnas aplastadas; se mantiene. Carrito cotizador `min-w-[680px]` (mismo patrón, aceptable); 15 tablas crudas fuera de DataTable varias sin `overflow-x-auto`; 21 grids `grid-cols-2/3` sin breakpoint en modales; **cotizador no usable en móvil** (header con 6+ acciones, pickers `min-w-[260px]+`).
